...
class LogAccessTest(APITestCase):
    def setUp(self):
        self.student_a = CustomUser.objects.create_user(username='student_a',email='student_a@example.com', password='testpass', role='student')
        self.user = CustomUser.objects.create_user(email="supervisor@test.com", password="testpassword123", role="SUPERVISOR")
        self.student_b = CustomUser.objects.create_user(username='student_b',email='student_b@example.com', password='testpass', role='student')
